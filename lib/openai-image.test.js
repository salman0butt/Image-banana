...
test("does not expose OpenAI authentication status directly to clients", async () => {
  const openAIImage = await import("./openai-image");
...
test("maps upstream failures to bad gateway", async () => {
  const openAIImage = await import("./openai-image");
...
test("maps timeouts to gateway timeout", async () => {
  const openAIImage = await import("./openai-image");

  expect(openAIImage.mapOpenAIError(new Error("Request timed out"), false)).toEqual({
