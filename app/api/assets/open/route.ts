...
import { ensureEditableAsset } from "@/lib/assets";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const editable = await ensureEditableAsset({
      userId,
      assetId,
      client,
    ...
    const missing = message.includes("not ready") || message.includes("No image asset");
    return NextResponse.json(
      {
        error: {
