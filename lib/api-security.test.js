...
test("rejects missing content-length when the endpoint requires it", () => {
  const request = new Request("http://localhost/api/test", {
    method: "POST",
  });

  try {
    assertRequestContentLength(request, 2048, true);
    throw new Error("Expected request to be rejected.");
  } catch (error) {
    expect(error).toBeInstanceOf(ApiRequestError);
...
test("does not expose unexpected internal error messages", async () => {
